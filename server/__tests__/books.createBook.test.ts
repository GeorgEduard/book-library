...
describe('createBook', () => {
  ...
  it('maps P2003 to friendly error when author_id invalid', async () => {
    (prisma.book.create as unknown as Mock).mockRejectedValue({
      code: 'P2003',
    });
    const res = mockRes();
    ...
    expect(res.body).toEqual({
      error: 'author_id does not reference an existing author',
    });
  });

  it('creates book and returns mapped fields', async () => {
    (prisma.book.create as unknown as Mock).mockResolvedValue({
      id: 10,
      title: 'A',
      authorId: 2,
      author: { name: 'Alice' },
